...
struct Ext2FSInode * Ext2FS::load_inode(unsigned int inode_number) {
    ...
    // Tengo el blockgroup de ese inodo
    unsigned int index_inode = blockgroup_for_inode(inode_number);

    // Tengo el offset de ese inodo
    unsigned int offset_inode = blockgroup_inode_index(inode_number);

    // Ahora necesito el group
    Ext2FSBlockGroupDescriptor *bgd_inode = block_group(index_inode);
	
	// Me defino un espacio en memoria para tener lo necesario del superbloque
	unsigned char * block_buffer = (unsigned char *) malloc(1024 << _superblock->log_block_size);

	read_block(bgd_inode->inode_table, block_buffer);

	// Tengo la tabla leida, ahora reviso con el inodo

	Ext2FSInode *inode = ((Ext2FSInode*) block_buffer[offset_inode]);
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number) {
    if(block_number < 12) {
        ...
        if(block_number < 256 + 12) {
            // Indirecto Simple
            int block_address = inode->block[12];
            read_block(block_address, block_buffer);
            unsigned int offset_ind = block_number - 12;

            result = (unsigned int*) block_buffer[offset_ind];
        } else {
            if(block_number < 256*256 + 256 + 12) {
                // Indirecto Doble
                int block_address = inode->block[13];
                read_block(block_address, block_buffer);
                // Tengo la tabla de punteros a tablas. Ahora ¿Que sigue?
                // Por cada uno de las filas tengo un puntero.
                // Cada puntero es hacia una tabla. Entonces, tengo que ver que tabla traer primero.
                // Cada tabla tiene 256 posiciones. Entonces. Tengo (valor - 256 - 12 % 256)
                // Esto me da el primer puntero para saber a que tabla tengo que ir
                int offset_ind_dual = (int) ((block_number - 256 - 12)/256);
                // Esto me dice de la tabla final a que posición tengo que acceder
                unsigned int offset_ind = (block_number - 256 - 12) % 256;

                unsigned char *block_buffer_dual = (unsigned char *) malloc(1024 << _superblock->log_block_size);
                read_block(block_buffer[offset_ind_dual], block_buffer_dual);

                result = (unsigned int* ) block_buffer_dual[offset_ind];
                free((int * ) block_buffer_dual);
            } else {
                // Indirecto triple
            }
        }
...
struct Ext2FSInode * Ext2FS::get_file_inode_from_dir_inode(struct Ext2FSInode * from, const char * filename)
{
	...
	while(!wasFind) {
		...
		int block_address = get_block_address(from, indice);

		read_block(block_address, block_buffer);

		Ext2FSDirEntry *dirEntry = (Ext2FSDirEntry *) block_buffer;
